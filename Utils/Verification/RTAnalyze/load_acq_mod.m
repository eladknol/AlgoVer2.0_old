...
%
function acq = load_acq_mod(filename, chan_by_chan)
...
if length(unique(sample_divider))==1 && unique(sample_divider)==1
    ...
    min_len = min(abs([hdr.per_chan_data.buf_length].*sample_divider));
end
...
if length(unique([hdr.per_chan_type.type])) && ~chan_by_chan && ...
    ...
    if length(unique(abs(sample_divider)))==1 && unique(abs(sample_divider))==1
...
%                 if hdr.per_chan_data(i).buf_length~=0
...
%                 else
                    tmp=fread(fid,'int16',skip_chan) ...
                        * hdr.per_chan_data(2).ampl_scale ...
                        + hdr.per_chan_data(2).ampl_offset;
%                 end
            end
...
%                 data(:,i) = tmp;
                if i==1;
                    data=zeros(size(tmp,1),hdr.graph.num_channels);
                end;
                data(1:length(tmp),i)=tmp;              
            end
        end
        ...
        for j = 1:hdr.graph.num_channels
            mask(1:abs(sample_divider(j)):min_len, j)=1;
        end
