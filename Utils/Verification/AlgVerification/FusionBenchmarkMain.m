...
res=FusionBenchmarkGenStruct(N_Subjects);
...
for i_Subject=1:N_Subjects
    ...
    [res.IndPerf(i_Subject).ECG_Session_det_vec,res.IndPerf(i_Subject).Audio_Session_det_vec...
        ,res.IndPerf(i_Subject).Fusion_Session_det_vec,res.IndPerf(i_Subject).Overlap_Session_det_vec...
        ,res.IndPerf(i_Subject).ECG_Session_score_vec,res.IndPerf(i_Subject).Audio_Session_score_vec...
        ,res.IndPerf(i_Subject).Included]=deal(zeros(1,N_Sessions));
    ...
    for i_Session=1:N_Sessions
        ...
        if isfield(res.IndPerf(i_Subject).NGO_Data{i_Session,1},'resData')==1
            ...
            [res.IndPerf(i_Subject).Included(i_Session),res.IndPerf(i_Subject).Exclusion_Reason{i_Session,1}]=AlgVerificationIncSessFlag(params.Bench,res.IndPerf(i_Subject).hdr{i_Session,1});
            ...
            if res.IndPerf(i_Subject).Included(1,i_Session)
                ...
                res.IndPerf(i_Subject).ECG_Interval_xcorr_arr{1,i_Session}=[res.IndPerf(i_Subject).NGO_Data{i_Session}.resData(1:N_Intervals).f_E_m_E_max]';
                res.IndPerf(i_Subject).Audio_Interval_xcorr_arr{1,i_Session}=[res.IndPerf(i_Subject).NGO_Data{i_Session}.resData(1:N_Intervals).f_A_m_E_max]';
                ...
                for i_Interval=1:N_Intervals
                    ...
                    %% Check if interval should be included in benchmark/verification according to alg. parameters( detection score, HR, xcorr, etc.)
                    [flag,res.IndPerf(i_Subject)]=AlgVerificationIncIntFlag(params.Verf,res.IndPerf(i_Subject),i_Session,i_Interval);
                    if flag
                ...
                %% Get Fusion and overlap results 
                ...
                % update total number of intervals with HR detected for each module, per subject
               ...
               % classify session as 'HR detected'  if at least one interval has a HR detection
                if any(res.IndPerf(i_Subject).ECG_Interval_det_arr{1,i_Session})
                    res.IndPerf(i_Subject).ECG_Session_det_vec(i_Session)=1;
                end
                
                if any(res.IndPerf(i_Subject).Audio_Interval_det_arr{1,i_Session})
                    res.IndPerf(i_Subject).Audio_Session_det_vec(i_Session)=1;
                end
                
                if any(res.IndPerf(i_Subject).Fusion_Interval_det_arr{1,i_Session})
                    res.IndPerf(i_Subject).Fusion_Session_det_vec(i_Session)=1;
                end
                
                if res.IndPerf(i_Subject).ECG_Session_det_vec(i_Session) && res.IndPerf(i_Subject).Audio_Session_det_vec(i_Session)
...
Out_Folder_Path=fullfile(handles.NGO_Folder_Path,['Stats_' datestr(datetime,'YYYY_mm_dd_HH_MM_SS')]);
mkdir(Out_Folder_Path);

% save parameters for run
AlgVerificationWriteParams(Out_Folder_Path,handles.params);
FusionBenchmarkGenStats(res.IndPerf(Included_Idx),Out_Folder_Path,Out_File_Str,'Sessions');
FusionBenchmarkGenStats(res.IndPerf(Included_Idx),Out_Folder_Path,Out_File_Str,'Intervals');
% FusionBenchmarkCalcAgmt(res.IndPerf(Included_Idx),Out_Folder_Path,Out_File_Str,'Intervals');
