...
%   None.
%
%% Example I- simply present found file names:
...
%   fprintf('%s\n', foldersList{:});
%
%% Example II- find oversized files:
% maxFileSize=10; %[Mb]
% fileNamesList=folderFiles([], Inf);
% filesData=cellfun(@dir, fileNamesList, 'UniformOutput', false);
% isEmptyCell=cellfun(@isempty, filesData, 'UniformOutput', false);
% isEmptyCellArr=cat(2, isEmptyCell{:});
% filesData=filesData(~isEmptyCellArr);
% fileNamesList=fileNamesList(~isEmptyCellArr);
% filesDataStructArr=cat(2, filesData{:});
% filesSizeMB=cat(2, filesDataStructArr.bytes)/1024/1024;
% 
% isFileOversized=(filesSizeMB > maxFileSize);
% fprintf('Oversized files:\n');
% iOverSizedFiles=find(isFileOversized);
% for iFile=iOverSizedFiles
%     fprintf( 'Size- %.1f [MB]\t%s\n', filesSizeMB(iFile),...
%         fileNamesList{iFile} );
% end
%
%% See also:
...
%
% *List of Changes:*
% 2013-04-28- replaced for by while loop to take care of nFolderDepth=Inf
...
iFolderLevel=1;
while iFolderLevel <= nFolderDepth  % Scan folders nFolderDepth 
