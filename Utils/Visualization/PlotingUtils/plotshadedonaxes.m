function varargout = plotshadedonaxes(axes,x,y,fstr)
% axes: 
% x: x coordinates
...
if size(y,1)==2 %plot shaded area
    ...
    py=[y(1,:), fliplr(y(2,:))];
    p=patch(px,py,1,'FaceColor',fstr,'EdgeColor','none','Parent',axes);
end;
...
if size(y,1)==3 % also draw mean
    ...
    py=[y(1,:), fliplr(y(3,:))];
    p=patch(px,py,1,'FaceColor',fstr,'EdgeColor','none','Parent',axes);
    plot(axes,x,y(2,:),fstr);
end;
 
alpha(p,.1); % make patch transparent