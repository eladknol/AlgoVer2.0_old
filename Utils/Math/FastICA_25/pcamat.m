function [E, D] = pcamat(vectors, firstEig, lastEig, s_interactive, s_verbose) %#codegen
...
if ~(b_interactive)
    if lastEig < 1 | lastEig > oldDimension
        if(coder.target('matlab'))
            ...
            error('Illegal value for parameter: ''lastEig''' );
        end
    end
    if firstEig < 1 | firstEig > lastEig
        if(coder.target('matlab'))
            ...
            error('Illegal value for parameter: ''firstEig''');
        end
...
if(coder.target('matlab'))
    ...
    numTaken = 0;
    takingMask = 1:length(maskVector);
